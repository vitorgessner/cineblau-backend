...
import { find, create, findWithDetails } from '../models/films.js';
...
export const getAllFilmsWithDetails = async (req: Request, res: Response) => {
    const films = await findWithDetails();
    if (films.length === 0) return res.status(404).json('no films found');
